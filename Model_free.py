'''
revuresnet.py taken from implementation of ShapeHD 
'''
...
weights_25d.load_state_dict(torch.load('models/first_module_.pth', map_location=torch.device('cpu')))
weights_3d = Model3d()
weights_3d.load_state_dict(torch.load('models/second_module.pth', map_location=torch.device('cpu')))

os.system('rm -rf 2p5d_results')
os.system('mkdir 2p5d_results')
os.system('rm -rf 3D_models')
os.system('mkdir 3D_models')
...
is_bg = sil_img1 <= 0
depth_img1[is_bg] = 0
normal_img1[is_bg.repeat(1, 3, 1, 1)] = 0
x = torch.cat((depth_img1, normal_img1), 1)
V = weights_3d(x)
...
thres = np.where(V<-1.2, 0, 1).squeeze()
import matplotlib.pyplot as plt
